...
import com.reportingtool.xml.dmo.DeclaracionPositiva;
...
/**
 * Class to generate XML string from a reportExecution type DMO_DP
 * 
 * @author Alberto
 * 
 */
public class GeneratorXMLDMO_DP {
	
	private static final String schemaRoot = "<Declaracion ";
	private static final String schemaRoot2 = "<ns2:Declaracion ";
	private static final String schemaRootEnd = "</Declaracion>";
	private static final String schemaRootEnd2 = "</ns2:Declaracion>";
	...
	public static GenerateXMLForm generateXMLDMO(
		...
		try {
			...
			DeclaracionPositiva declaracionPositiva = objectFactory
					.createDeclaracionPositiva();

			// (1) <ID>
			if (ReportUtilities.searchData(reportDatas, "ID", "1", null) != null)
				declaracionPositiva.setID(ReportUtilities.searchData(
						reportDatas, "ID", "1", null));

			// /////////////////////////////////////////////////////////////////
			// DeclaracionPositiva

			DeclaracionPositiva.DatosEntrada datosEntrada = objectFactory
					.createDeclaracionPositivaDatosEntrada();
			...
			// <Operaciones>
			DeclaracionPositiva.Operaciones operaciones = objectFactory
					.createDeclaracionPositivaOperaciones();
			...
			declaracionPositiva.setDatosEntrada(datosEntrada);
			// declaracionPositiva.setID(id);
			declaracionPositiva.setOperaciones(operaciones);
			declaracionPositiva.setTYPE(TIPODECLENUM.POSITIVA);

			// final set to Declaracion
			declaracion.setDeclaracionPositiva(declaracionPositiva);
			...
			String resultFixed = st.toString().replace(schemaRoot, schemaRoot2);
			resultFixed = resultFixed.replace(schemaRootEnd, schemaRootEnd2);
