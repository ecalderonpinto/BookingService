...
public class InstallDMO_DM implements InstallAdapter {

	private ApplicationContext applicationContext;

	private static final Logger logger = LoggerFactory
			.getLogger(InstallDMO_DM.class);

	public InstallDMO_DM(ApplicationContext applicationContext) {
	...
	public void install() {
		...
		try {
			...
			ReportCatalog reportCatalog = new ReportCatalog(versionField,
					ReportCatalogLevelEnum.DEPARTMENT.getReportLevel(), ReportUtilities.reportCatalogDMO_DM_Name, "", null, null, null, versionAdmin);
			...
			ReportField reportFieldx1 = new ReportField(reportCatalog, null,
					"X", "DeclaracionMovimientos", new BigInteger("0"), null,
			...
			logger.error("Error in installation DMO_DM: " + e.getMessage());
		}
